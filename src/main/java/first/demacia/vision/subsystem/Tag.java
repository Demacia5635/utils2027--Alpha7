...
public class Tag extends SubsystemBase {
  ...
  private Supplier<ChassisVelocities> velocities;
   ...
   * orientation
   */
  public Tag(Supplier<Rotation2d> getRobotAngle, Supplier<ChassisVelocities> velocities, CameraConfig camera) {
  ...
  public Translation2d getOriginToRobot() {
    ...
    if (origintoTag != null) {
      ...
      originToRobot = origintoTag.plus(robotToTagFC.rotateBy(Rotation2d.PI));
...
private void crop() {
    ...
    private double getCropOfset() {
      ...
      return Math.clamp(crop, MIN_CROP, MAX_CROP);
    }

    private double getYawCrop(){
      double TagYaw = ((-camToTagYaw) + camera.getYaw()) / 31.25;
      return TagYaw + velocities.get().vy*PREDICT_Y + velocities.get().omega*PREDICT_OMEGA;
    }

    private double getPitchCrop(){
      double TagPitch = camToTagPitch / 24.45;
      return TagPitch + velocities.get().vx*PREDICT_X;
    }
